...
const STAGING = '/Users/egor/Desktop/Claude Code/wordpunk/data/staging/wave3';
...
for (const f of readdirSync(STAGING).sort()) {
  ...
  for (const line of lines) {
    ...
    if (cells.length < 5) {
      console.error(`[${f}] skip short row: ${line}`);
      fixed.push(line);
      continue;
    }
    // cells.length > 5: merge middle cells as sentence
    ...
    if (!/^[1-5]$/.test(diff) || !VALID_TOPICS.has(topic)) {
      console.error(`[${f}] can't fix: diff=${diff} topic=${topic} :: ${line}`);
      fixed.push(line);
      continue;
    }
    const middle = cells.slice(2, cells.length - 2).map(s => s.trim()).join(', ');
    const row = [en, ru, csvEscape(middle), diff, topic].join(',');
    fixed.push(row);
    changes++;
  }
